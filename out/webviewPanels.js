...
exports.CalculatorPanel = exports.RatesViewPanel = void 0;
const vscode = __importStar(require("vscode"));
class RatesViewPanel {
    constructor(service) {
     ...
     * 显示汇率查看面板
     */
    async showRates(baseCurrency = 'USD') {
        if (this.panel) {
            this.panel.reveal();
        ...
        this.panel = vscode.window.createWebviewPanel('currencyRates', title, vscode.ViewColumn.One, {
            enableScripts: true,
     ...
     * 更新汇率视图
     */
    async updateRatesView(baseCurrency = 'USD') {
        if (!this.panel) {
            return;
        }
    ...
    async refreshHistoryPanel() {
        if (this.historyPanel && this.historyPanelData) {
            ...
            try {
                ...
                this.historyPanel.webview.html = this.getErrorHtml(error instanceof Error ? error.message : this.t('history.loadFailed'));
            }
        }
    }
    /**
     ...
     * 显示历史汇率面板
     */
    async showHistoryPanel(currencyPair) {
        ...
        const panel = vscode.window.createWebviewPanel('currencyHistory', title, vscode.ViewColumn.One, {
            enableScripts: true,
            retainContextWhenHidden: true
        });
        // 保存面板引用和数据
        ...
        try {
            ...
            panel.webview.html = this.getErrorHtml(error instanceof Error ? error.message : this.t('history.loadFailed'));
        }
        // 处理来自webview的消息
        panel.webview.onDidReceiveMessage(async (message) => {
            if (message.command === 'refresh') {
                ...
                try {
                    ...
                    panel.webview.html = this.getErrorHtml(error instanceof Error ? error.message : this.t('history.refreshFailed'));
                }
            }
    ...
    getHistoryHtml(data) {
        ...
        // 生成表格行
        const tableRows = data.rates.map((item) => {
...
exports.RatesViewPanel = RatesViewPanel;
class CalculatorPanel {
    ...
    async showCalculator() {
        ...
        this.panel = vscode.window.createWebviewPanel('currencyCalculator', title, vscode.ViewColumn.One, {
            enableScripts: true,
            retainContextWhenHidden: true
        });
        this.panel.onDidDispose(() => {
        ...
        this.panel.webview.onDidReceiveMessage(async (message) => {
            if (message.command === 'convert') {
                try {
                    const result = await this.service.convert(message.amount, message.from, message.to);
