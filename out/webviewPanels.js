...
class RatesViewPanel {
    ...
    async showHistoryPanel(currencyPair) {
        ...
        const panel = vscode.window.createWebviewPanel('currencyHistory', `${currencyPair} 历史汇率`, vscode.ViewColumn.One, {
            enableScripts: true,
            retainContextWhenHidden: true
        });
        // 显示加载状态
        ...
        try {
            ...
            panel.webview.html = this.getErrorHtml(error instanceof Error ? error.message : '获取历史汇率失败');
        }
        // 处理来自webview的消息
        panel.webview.onDidReceiveMessage(async (message) => {
            if (message.command === 'refresh') {
                ...
                try {
                    ...
                    panel.webview.html = this.getErrorHtml(error instanceof Error ? error.message : '刷新失败');
                }
            }
                ...
                try {
                    ...
                    panel.webview.html = this.getErrorHtml(error instanceof Error ? error.message : '加载失败');
                }
            }
